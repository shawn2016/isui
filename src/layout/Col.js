...
export default class Col extends Component {
  render() {
    ...
    ['span', 'offset', 'pull', 'push', 'order'].forEach((prop) => {
      ...
      if (num) {
        classList.push(
          prop !== 'span' ? `is-col-${prop}-${num}` : `is-col-${num}`
        );
      }
    });
    ...
    ['xs', 'sm', 'md', 'lg'].forEach((size) => {
      if (typeof this.props[size] === 'object') {
        ...
        Object.keys(props).forEach((prop) => {
          classList.push(
            prop !== 'span'
              ? `is-col-${size}-${prop}-${props[prop]}`
              : `is-col-${size}-${props[prop]}`
          );
        });
      } else if (this.props[size]) {
        classList.push(`is-col-${size}-${Number(this.props[size])}`);
      }
    });
    ...
    return createElement(this.props.tag, {
      className: this.classNames('is-col', classList), ...others,
    }, this.props.children);
  }
}
