...
describe('<Tree>', () => {
  ...
  it('Test the default props and node.', () => {
    ...
    // default props test
    expect(wrapper.find('.is-tree')).toHaveLength(1);
    expect(wrapper.type()).toEqual(Tree);
    expect(wrapper.at(0).prop('prefixCls')).toBe('is-tree');
    ...
    expect(wrapper.find('.is-tree').type()).toBe('div');
  });

  it('Test the showLine props.', () => {
    wrapper.setProps({ showLine: true });
    expect(wrapper.find('.is-tree ul').at(0).hasClass('is-tree-show-line')).toBe(true);
  });

  it('Test the selectedKeys props.', () => {
    wrapper.setProps({ selectedKeys: ['0-0-0'] });
    expect(wrapper.prop('selectedKeys')).toEqual(['0-0-0']);
    expect(wrapper.find('.is-tree .is-tree-selected')).toHaveLength(1);
  });
  ...
  it('Test the data and option props.', () => {
    ...
    expect(wrapper.find('.is-tree .is-tree-inner').at(0).text()).toBe('上海市');
    expect(wrapper.find('.is-tree li ul').at(0).children()).toHaveLength(2);
  });
});
