...
describe('<Collapse>', () => {
  ...
  it('Test the default props and node.', () => {
    ...
    // 默认值测试
    expect(wrapper.find('.is-collapse')).toHaveLength(1);
    ...
    expect(wrapper.find('.is-collapse-item').at(0).prop('className')).toBe('is-collapse-item');
    expect(wrapper.find('.is-collapse-item').at(1).prop('className')).toBe('is-collapse-item is-collapse-active');
  });

  it('Test onChange event.', () => {
    const wrapper1 = wrapper.find('.is-collapse-item').at(2).find('.is-collapse-header').at(0);
