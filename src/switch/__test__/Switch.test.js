...
describe('<Switch>', () => {
  ...
  it('Test the default props and node.', () => {
    ...
    // 默认值测试
    expect(wrapper.find('.is-switch')).toHaveLength(1);
  ...
  it('Test checked attributes.', () => {
    expect(wrapper.find('.is-switch').at(0).prop('className')).toBe('w-switch w-switch-default');
    wrapper.setProps({ checked: true });
    expect(wrapper.find('.is-switch').at(0).prop('className')).toBe('w-switch w-switch-checked w-switch-default');
  });

  it('Test size attributes.', () => {
    wrapper.setProps({ size: 'large' });
    expect(wrapper.find('.is-switch').at(0).prop('className')).toBe('w-switch w-switch-checked w-switch-large');
  });

  it('Test onChange event.', () => {
    const S = wrapper.find('.is-switch input').at(0);
    S.simulate('change');
