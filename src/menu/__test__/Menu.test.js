...
describe('<Menu>', () => {
  ...
  it('Test the default props and node.', () => {
    ...
    // 默认值测试
    expect(wrapper.find('.is-menu')).toHaveLength(1);
  ...
  it('Test mode attributes.', () => {
    ...
    expect(wrapper.find('.is-menu').at(0).prop('className')).toBe('w-menu w-menu-vertical');
  });

  it('Test onSelect event.', () => {
    const menu = wrapper.find('.is-menu li').at(1);
    menu.simulate('click');
