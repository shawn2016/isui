...
describe('<Tag>', () => {
  ...
  it('Test the default props and node.', () => {
    ...
    // 默认值测试
    expect(wrapper.find('.is-tag')).toHaveLength(1);
  ...
  it('Test value attributes.', () => {
    ...
    expect(wrapper.find('.is-tag').at(0).prop('className')).toBe('w-tag w-tag-red');
    wrapper.setProps({ color: 'default' });
    expect(wrapper.find('.is-tag').at(0).prop('className')).toBe('w-tag w-tag-white');
  });
});
...
describe('<Tag.Group>', () => {
  ...
  it('Test onChange func', () => {
    ...
    // 默认值测试
    expect(wrapper1.find('.is-tag-group')).toHaveLength(1);
  ...
  it('Test onChange event.', () => {
    const T = wrapper1.find('.is-tag').at(1);
    T.simulate('click');
