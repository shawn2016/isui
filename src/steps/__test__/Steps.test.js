...
describe('<Steps>', () => {
  ...
  it('Test the default props and node.', () => {
    ...
    // 默认值测试
    expect(wrapper.find('.is-steps')).toHaveLength(1);
  ...
  it('Test status attributes', () => {
    ...
    expect(wrapper.find('.is-steps-item').at(2).prop('className')).toBe('w-steps-item w-steps-item-error');
  });

  it('Test direction attributes', () => {
    wrapper.setProps({ direction: 'vertical' });
    expect(wrapper.find('.is-steps').at(0).prop('className')).toBe('w-steps w-steps-vertical');
  });
});
