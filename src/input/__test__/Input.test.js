...
describe('<Input>', () => {
  ...
  it('Test the default props and node.', () => {
    ...
    // 默认值测试
    expect(wrapper.find('.is-input')).toHaveLength(1);
    expect(wrapper.type()).toEqual(Input);
    expect(wrapper.at(0).prop('prefixCls')).toBe('is-input');
    ...
    expect(wrapper.find('.is-input-inner').type()).toBe('input');
  });


  it('Test disabled attributes.', () => {
    wrapper.setProps({ disabled: true });
    expect(wrapper.find('.is-input').at(0).prop('className')).toBe('is-input w-disabled');
  });
  ...
  it('Test textarea attributes.', () => {
    ...
    expect(wrapper.find('.is-input').at(0).prop('className')).toBe('is-input textarea is-input-inner');
  });

  it('Test onChange event.', () => {
    wrapper.setProps({ type: 'text' });
    const input = wrapper.find('.is-input').at(0);
    input.simulate('change');
    expect(input.html()).toContain('<div class="is-input"><input type="text" autocomplete="off" class="is-input-inner"></div>');
  });
  ...
  it('Test onSearch event.', () => {
    ...
    const input = wrapper.find('.is-input').at(0);
    input.simulate('change');
    expect(input.html()).toContain('<div class="is-input"><input type="input" autocomplete="off" class="is-input-inner"></div>');
  });
