...
describe('Message', () => {
  it('Test placement attributes', () => {
    ...
    expect(document.querySelectorAll('.is-message')[0].className).toBe('w-message w-message-bottom');
    expect(document.querySelectorAll('.is-message').length).toBe(1);
  });
  it('Test Message.success', () => {
    Message.success('success', { placement: 'topLeft' });
    expect(document.querySelectorAll('.is-message').length).toBe(1);
    expect(document.querySelectorAll('.is-message')[0].className).toBe('w-message w-message-top-left');
  });
