...
describe('<Transition>', () => {
  ...
  it('Test default porps and node.', () => {
    expect(wrapper.find('.is-animate')).toHaveLength(2);
    expect(wrapper.type()).toEqual(Transition);
    expect(wrapper.at(0).prop('prefixCls')).toBe('is-animate');
    ...
    expect(wrapper.find('.is-animate').at(0).html()).toContain('<div class="is-animate is-fadeIn is-down is-mounting">向下淡进向上淡出</div>');
  });
});
