...
describe('<Select>', () => {
  ...
  it('Test the default props and node.', () => {
    ...
    // 默认值测试
    expect(wrapper.find('.is-select')).toHaveLength(1);
    expect(wrapper.type()).toEqual(Select);
    expect(wrapper.at(0).prop('prefixCls')).toBe('is-select');
  ...
  it('Test onChange func', () => {
    const select = wrapper.find('.is-select').at(0).find('.is-input').at(0);
