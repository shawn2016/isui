...
describe('<Progress>', () => {
  ...
  it('Test the default props and node.', () => {
    ...
    // 默认值测试
    expect(wrapper.find('.is-progress')).toHaveLength(1);
    ...
    expect(wrapper.find('.is-progress').at(0).prop('className')).toBe('w-progress w-progress-line w-progress-show-text');
  });

  it('Test type attributes.', () => {
    wrapper.setProps({ type: 'circle' });
    expect(wrapper.find('.is-progress').at(0).prop('className')).toBe('w-progress w-progress-circle w-progress-show-text');
  });

  it('Test status attributes.', () => {
    wrapper.setProps({ status: 'active' });
    expect(wrapper.find('.is-progress').at(0).prop('className')).toBe('w-progress w-progress-circle w-progress-show-text w-progress-status-active');
    wrapper.setProps({ status: 'exception' });
    expect(wrapper.find('.is-progress').at(0).prop('className')).toBe('w-progress w-progress-circle w-progress-show-text w-progress-status-exception');
