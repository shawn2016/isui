...
describe('<Tabs>', () => {
  ...
  it('Test the default props and node.', () => {
    ...
    // 默认值测试
    expect(wrapper.find('.is-tabs')).toHaveLength(1);
    expect(wrapper.type()).toEqual(Tabs);
    expect(wrapper.at(0).prop('prefixCls')).toBe('is-tabs');
    ...
    expect(wrapper.find('.is-active').at(0).html()).toContain('<div class="is-animate is-mounting is-tabs-tab w-disabled w-active">配置管理</div>');
  });

  it('Test onTabClick event.', () => {
    const T = wrapper.find('.is-tabs-tab').at(2);
    T.simulate('click');
    expect(wrapperState.value).toBe('3');
    expect(wrapper.find('.is-active').at(0).html()).toContain('<div class="is-animate is-mounting is-tabs-tab w-active">角色管理</div>');
  });
});
