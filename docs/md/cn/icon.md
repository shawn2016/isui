...
语义化的矢量图形，内置的图标属于UI框架常用图形字体。icon字体以及样式，被抽离到一个新的仓库 [isui icon](https://isui-react.github.io/icons/) ，`isui`去依赖 [isui-iconfont](https://github.com/isui-react/icons)，这个仓库主要是维护一套svg图片，并将svg图片转换为 `*.svg` `*.ttf` `*.woff` `*.eot` 等字体及相关文件并发布到 [npm](https://www.npmjs.com/package/isui-iconfont) 上去。
...
`icon` 名字需要命名规范，必须加上前缀 `.is-icon-{图标名字}` 。
...
[class^="is-icon-isui-"], [class*=" is-icon-isui-"] {
...
.is-icon-isui-wxbgongju:before { content: "\e61b"; }
.is-icon-isui-wxbmingxingdianpu:before { content: "\e61c"; }
```

上面写好CSS之后在组件中引用，就可以通过 `Icon` 组件来调用了，例如上面定义了两个图标使用方法如下

```js
<Icon type="isui-wxbgongju" />
<Icon type="isui-wxbmingxingdianp" />
```

注意：这里 `.is-icon-` 是当前默认引用的字体文件，`is-icon-isui-` 是引用自己制作的字体文件，来覆盖默认的字体文件，就当成一个新的作用域。
...
> 描线：`-o` 图标添加边框描线
> 默认：`is-icon-` 默认引用的字体文件作用域  
> 自定定义：`is-icon-isui-` 自定定义作用域  
