...
| prefixCls | 默认样式`is-animate` | String | is-animate |
...
| prefixCls | 自定义过渡效果。自带过渡效果，时间设置是没有用的，通过这个来自定义一个样式，默认样式 “is-animate”，自定义样式会拼接成 “is-animate-fade-in”，在CSS样式里面，根据react-transition-group规则最终需要这样写样式，".is-animate-fade-in-leave" | String | is-animate |