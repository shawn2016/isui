...
语义化的矢量图形，内置的图标属于UI框架常用图形字体。icon字体以及样式，被抽离到一个新的仓库 [uiw icon](https://github.com/shawn2016/isui.git/icons/) ，`uiw`去依赖 [isui-iconfont](https://github.com/uiw-react/icons)，这个仓库主要是维护一套svg图片，并将svg图片转换为 `*.svg` `*.ttf` `*.woff` `*.eot` 等字体及相关文件并发布到 [npm](https://www.npmjs.com/package/isui-iconfont) 上去。
...
[class^="is-icon-uiw-"], [class*=" is-icon-uiw-"] {
...
.is-icon-uiw-wxbgongju:before { content: "\e61b"; }
.is-icon-uiw-wxbmingxingdianpu:before { content: "\e61c"; }
```
...
注意：这里 `.is-icon-` 是当前默认引用的字体文件，`is-icon-uiw-` 是引用自己制作的字体文件，来覆盖默认的字体文件，就当成一个新的作用域。
